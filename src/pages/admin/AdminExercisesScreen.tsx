...
export default function AdminExercisesScreen() {
  ...
  const handleAddMistake = () => {
    if (
      mistakeInput.trim() &&
      !formData.commonMistakes.includes(mistakeInput.trim())
    ) {
      setFormData({
        ...formData,
  ...
  const confirmDeleteExercise = () => {
    if (exerciseToDelete) {
      setExercisesList(
        exercisesList.filter((e) => e.id !== exerciseToDelete.id),
      )
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <Modal
        ...
        title={
          editingExerciseId ? 'Editar Ejercicio' : 'Agregar Nuevo Ejercicio'
        }
      >
          ...
          <div className="mb-4">
            <label className="block text-foreground text-sm font-medium mb-2">
              ...
              <span className="text-muted-foreground text-xs">
                ({formData.muscleGroups.length} seleccionados)
              </span>
            </label>
            ...
            {errors.muscleGroups && (
              <p className="text-cinnabar text-xs mt-1">
                {errors.muscleGroups}
              </p>
            )}
          </div>

...
export default function ExercisesScreen() {
  ...
  const filteredExercises = exercises.filter((ex) => {
    ...
    const matchesMuscle =
      selectedMuscle === 'all' || ex.muscleGroups.includes(selectedMuscle)
  ...
  const getLevelColor = (level: Level) => {
    switch (level) {
      case 'beginner':
        return 'bg-medium-jungle'
      case 'intermediate':
        return 'bg-cinnabar'
      case 'advanced':
        return 'bg-purple-600'
      default:
        return 'bg-cool-steel'
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="bg-background pt-12 pb-4 px-6">
        ...
        <div
          className="border border-border rounded-lg px-4 py-3 flex items-center mb-4"
            ...
            <div className="flex items-center">
              <div className="flex-1">
                <div className="flex items-center mb-2">
                  <h3 className="text-foreground text-lg font-semibold mr-2">
                    {exercise.name}
                  </h3>
                  <span
                    className={`px-2 py-1 rounded text-xs font-medium text-foreground ${getLevelColor(exercise.level)}`}
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  {exercise.muscleGroups.slice(0, 3).map((muscle) => (
                    <span
                      key={muscle}
                      className="bg-background/50 px-2 py-1 rounded text-xs text-muted-foreground"
                    >
                      {muscle}
                    </span>
                  ))}
